...
import {
  ...
  selectSinglePlayerBonusMovePresentation,
...
function App() {
  ...
  function gridPointFromClient(clientX, clientY, { clampToBoard = true, allowGoalGrid = false } = {}) {
    ...
    if (!clampToBoard) {
      const minX = allowGoalGrid ? -Number(settings.goalDepth) : 0;
      const maxX = allowGoalGrid ? Number(settings.cols) + Number(settings.goalDepth) : Number(settings.cols);
      if (rawX < minX || rawX >= maxX || rawY < 0 || rawY >= settings.rows) return null;
  ...
  function illegalMoveMessage(result) {
    ...
    else if (result.reason === "shot-target-must-be-opponent-goal") primary = <>Please select a cell inside the opponent&apos;s goal.</>;
  ...
  function onPiecePointerDown(pieceId, e) {
    ...
    if (["pass", "through-ball", "lofted-through-ball", "shot"].includes(actionResolutionRef.current?.kind)) {
  ...
  const passPreview = useMemo(() => {
    ...
    if (pending?.kind === "shot" && pending.shooterId && (pending.target || pending.attemptedTarget)) {
      ...
      const targetX = pending.target
        ? pending.target.side === "right" ? Number(settings.cols) + Number(pending.target.depth) : -Number(settings.goalDepth) + Number(pending.target.depth)
        : Number(pending.attemptedTarget.x);
      const targetY = pending.target ? goalTop + Number(pending.target.y) : Number(pending.attemptedTarget.y);
      const target = pending.status === "route-selection" || pending.targetInvalidReason
        ? { ...(pending.target || pending.attemptedTarget), renderX: targetX, renderY: targetY }
        : null;
      return {
        extendsGoals: true,
        ...
        target,
        targetStatus: pending.targetInvalidReason ? "blocked" : "clear",
        visibleCells: [],
        blockedCells: [],
  ...
  const passTargetDistance = useMemo(() => {
    const pending = actionResolution;
    if (!["pass", "through-ball", "lofted-through-ball", "shot"].includes(pending?.kind) || pending.status !== "targeting" || !hoveredCell) return null;
    const isShot = pending.kind === "shot";
    if (hoveredCell.y < 0 || hoveredCell.y >= settings.rows || (!isShot && (hoveredCell.x < 0 || hoveredCell.x >= settings.cols))) return null;
    const passer = pieces.find(piece => piece.id === (isShot ? pending.shooterId : pending.passerId));
    if (!passer) return null;
    const distance = Math.hypot((hoveredCell.x + .5) - (passer.x + .5), (hoveredCell.y + .5) - (passer.y + .5));
    if (isShot) {
      const shotRules = matchContextRef.current?.ruleSet?.actions?.shot || {};
      const normalMax = Number(shotRules.longShotNormalRangeMax) || 11;
      const maximum = Number(shotRules.shotMaximumRange) || 16;
      const insidePenalty = Number(passer.x) < Number(settings.boxDepth) || Number(passer.x) >= Number(settings.cols) - Number(settings.boxDepth);
      const label = distance > maximum ? "MAX" : insidePenalty ? "FIN" : distance <= normalMax ? "LS" : "DLS";
      return { x: hoveredCell.x, y: hoveredCell.y, label: `${distance.toFixed(2)} ${label}` };
    }
  ...
  }, [actionResolution, hoveredCell, pieces, settings.cols, settings.rows, settings.boxDepth, gameTimeline?.cursor]);
  ...
  const movementPreview = useMemo(() => {
    ...
    const bonusMove = !sessionCode && gameMode === "match" && actionContinuation?.kind === "bonus-card-action"
      ...
      && (actionContinuation.status !== CONTINUATION_STATUS.ACTION_ACTIVE || (actionContinuation.actionType === "MOVE" && String(actionContinuation.pieceId || "") === String(selectedPiece.id)));
    const result = bonusMove
      ? selectSinglePlayerBonusMovePresentation(currentTimelineGameStateSnapshot() || captureTimelineGameState(), singlePlayerMatchContext(), { piece: selectedPiece, x: hoveredCell.x, y: hoveredCell.y })
      : evaluateMove(selectedPiece, hoveredCell.x, hoveredCell.y);
    // Geometry is projected by the Engine. It can be null only for an invalid
    // presentation request, which must render as an illegal hover rather than
    // crashing the React root.
    const geometry = result.geometry;
    const axisIcon = movementAxisSymbol(geometry?.axis);
    if (gameMode === "editor") {
      return {
        ...result,
        label: geometry?.kind === "mixed" ? "—" : `${axisIcon ? `${axisIcon} ` : ""}${geometry?.cost ?? "—"}`,
      };
    }
    if (!result.legal) {
      return {
  ...
  function moveBoardPan(e) {
    ...
    if (["pass", "through-ball", "shot"].includes(pass?.kind) && pass.status !== "targeting") {
      setHoveredCell(null);
    } else if ((selectedId || activeInteractionPieceId) && e.pointerType !== "touch") {
      setHoveredCell(gridPointFromClient(e.clientX, e.clientY, { clampToBoard: false, allowGoalGrid: pass?.kind === "shot" && pass.status === "targeting" }));
    }
  ...
  function chooseShotTarget(target) {
    ...
    const dispatched = dispatchSinglePlayerGameCommand({
      ...
      label: `Shot target selected: ${Number(target.x) + 1}/${Number(target.y) + 1}`,
    });
    if (!dispatched.result.accepted) { setIllegalMoveNotice({ reason: dispatched.result.reason }); return false; }
    if (dispatched.state.actionResolution?.targetInvalidReason === "SHOT_TARGET_MUST_BE_OPPONENT_GOAL") setIllegalMoveNotice({ reason: "shot-target-must-be-opponent-goal" });
    else setIllegalMoveNotice(null);
    setHoveredCell(null);
    return true;
  }
  ...
  async function consumeInspectorAction(type, piece) {
    ...
    if (continuation) {
      ...
      if (!sessionCode && type === "THROUGH_BALL") {
        cancelFreeBall();
        beginThroughBallTargeting(piece);
        return;
      }
      if (!sessionCode && type === "LOFTED_THROUGH_BALL") {
        cancelFreeBall();
  ...
  function onBoardTouchEnd(e) {
    ...
    if (gesture?.mode === "one-finger" && e.touches.length === 0) {
      if (!gesture.moved && Date.now() >= multiTouchUntilRef.current) {
        ...
        } else if (actionResolutionRef.current?.kind === "shot" && actionResolutionRef.current.status === "targeting") {
          if (!canControlActiveResolution(actionResolutionRef.current)) return;
          const point = gridPointFromClient(gesture.startX, gesture.startY, { clampToBoard: false, allowGoalGrid: true });
          if (point) chooseShotTarget(point);
        ...
        } else if (["pass", "shot"].includes(actionResolutionRef.current?.kind)) {
  ...
  return (
    <div className={`app ${touchMode ? "touch-mode" : ""} ${lockUI ? "locked-ui" : ""} ${isReplayView ? "replay-mode" : ""} ${!sessionCode && gameMode === "match" ? "match-ui-presentation" : ""}`}>
      ...
      <BoardCanvas
        ...
        pitchRef={pitchRef}
        presentationMode={!sessionCode && gameMode === "match" ? "match" : "editor"}
                  ...
                  <div className="inspector-card-zoom-block">
                    <div className="inspector-card-zoom-tools">
                      ...
                      {!sessionCode && gameMode === "match" && (() => {
                        ...
                        const through = selectSinglePlayerInspectorActionPresentation(
                          currentTimelineGameStateSnapshot() || captureTimelineGameState(),
                          singlePlayerMatchContext(),
