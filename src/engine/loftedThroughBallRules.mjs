...
import { PASS_CORNERS, bodyBlockingPassOrigin, cardStat, defensiveCellsForPiece, pointForPassOrigin, pointForPassTarget, segmentIntersectsOpenRect, teamKeyForPiece } from "../rules/passEngine.mjs";
...
export function planLoftedThroughBall(state, context, passer, target, cornerId) {
  ...
  return {
    ...
    rollStatId: stat.id, rollStatValue: stat.value,
...
export function selectLoftedThroughBallTarget(state, context, command) {
  ...
  const cornerIds = context.ruleSet.actions.pass.pathMode === "center-to-center" ? [null] : PASS_CORNERS.map(corner => corner.id);
...
export function commitLoftedThroughBall(state, context, command) {
  const pending = state.actionResolution;
  const passer = state.pieces.find(piece => String(piece?.id) === String(pending?.passerId)) || null;
  const cornerId = context.ruleSet.actions.pass.pathMode === "center-to-center" ? null : String(command.payload?.cornerId || "");
...
function failureCandidates(state, team, passer) {
  const defenders = state.pieces.filter(piece => teamKeyForPiece(piece) === otherTeam(team) && !piece.inactive).map(piece => ({ piece, distance: distance(piece, passer) }));
  const best = defenders.length ? Math.min(...defenders.map(item => item.distance)) : null;
  return defenders.filter(item => Math.abs(item.distance - best) < EPSILON);
}
...
export function submitLoftedThroughBallRoll(state, context, command) {
  ...
  const disadvantage = resolveDiceModifierStacks(context.ruleSet.diceModifiers, "disadvantage", pending.plan.disadvantageStacks);
  const bonus = modifierType ? resolveDiceModifierStacks(context.ruleSet.diceModifiers, modifierType) : 0;
  const total = Number(event.natural) + Number(pending.plan.rollStatValue) + disadvantage + bonus;
...
export function resolveLoftedThroughBall(state, context) {
  ...
  const candidates = failureCandidates(state, pending.team, passer).map(item => ({ pieceId: item.piece.id, distance: item.distance }));
