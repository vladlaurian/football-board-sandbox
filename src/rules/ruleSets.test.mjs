...
test("Rule Set normalization preserves an explicit zero modifier cap", () => {
  ...
  assert.equal(normalized.diceModifiers.stackCap, 0);
});
...
test("Interception configuration normalizes independent modifier toggles", () => {
  ...
  assert.equal(normalized.diceModifiers.stackCap, 7);
});
...
test("Pass maximum distance is frozen no lower than the Long Pass threshold", () => {
