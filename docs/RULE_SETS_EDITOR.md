...
Long Pass checks its origin group first and its destination group second. A defender is eligible only when their defensive area contains the passer/actual receiver cell **and** the defender has a clear line to the real launch/landing point of the ball. This is the same visibility principle as Short Pass: an attacking body blocks only if that defender-to-ball line actually crosses its occupied cell. A receiver is therefore not automatically a blocker merely for occupying the destination; a lateral defender may still reach the arriving ball, while a body truly between defender and arrival point prevents the roll. Both groups are one progressive Interception sequence; stacks and carried Natural-1 disadvantage do not restart at reception.
