...
Long Pass checks its origin reaction zone first and its destination reaction zone second. In either local zone, a defender is eligible when the actual selected-corner route crosses one of that defender's defensive-area cells **and** the defender has a clear line to the ball in that concrete crossed cell. This is the same visibility principle as Short Pass: an attacking body blocks only if that defender-to-ball line actually crosses its occupied cell. A receiver is therefore not automatically a blocker merely for occupying the destination; a lateral defender may still reach the arriving ball, while a body truly between defender and arrival point prevents the roll. The aerial middle creates no interceptor. Both groups are one progressive Interception sequence; stacks and carried Natural-1 disadvantage do not restart at reception.
