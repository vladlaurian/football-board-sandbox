...
Every gameplay distance measurement is a named Engine fact measured square-centre to square-centre. Visual execution points such as Pass corners may affect a route, foot, body contact or badge, but never a distance threshold, Short/Long classification or difficulty calculation. The frozen MatchContext Rule Set supplies every threshold and selected stat ID. The canonical Pass plan records its classification, distance, route facts, interceptor groups and attacker target value before Timeline receives `PASS_CONFIRMED`.
