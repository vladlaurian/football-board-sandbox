...
Every gameplay distance measurement is a named Engine fact measured square-centre to square-centre. Visual execution points such as Pass corners may affect a route, foot, body contact or badge, but never a distance threshold, Short/Long classification or difficulty calculation. The frozen MatchContext Rule Set supplies every threshold and selected stat ID. The canonical Pass plan records its classification, distance, direct contact, route facts, interceptor groups/reaction points and attacker target value before Timeline receives `PASS_CONFIRMED`. A Long plan also stores endpoint-local `longReactionZones`, their crossed defensive cells and the global origin-before-destination interceptor sequence; the remaining route is aerial. The official Single Player Pass projection owns the corresponding Short/Long segmented route and one shared route verdict; UI never rediscovers the contact or assigns a second colour. The selected target is a normal receiver rather than an intermediate contact, so only an earlier physical body contact produces a grey continuation.
