...
## ADR-012 — Match-only action affordances remain mode-scoped

**Status:** Active

**Decision:** Restrictions that belong to Match gameplay must be applied only in Match Mode and must not alter Editor/Sandbox freedom. PASS possession eligibility is checked at both the Inspector UI boundary and the action-entry boundary, but the Pass Engine and interception rules remain unchanged. `Free Ball` is a one-shot Match control: it authorizes one manual ball relocation, then clears selection and exits automatically.

**Consequences:**

- A player without the ball cannot start PASS in Match Mode.
- Editor/Sandbox behavior is not globally disabled by Match-only eligibility checks.
- Free Ball cannot become a persistent selection state.
- Existing Pass interception rolls and the 3/2 rule remain authoritative.
- Visible release version, package version, README release heading, and archive name must identify the same build.
- Delivery ZIP files exclude `package-lock.json`, `node_modules`, and generated build output.
