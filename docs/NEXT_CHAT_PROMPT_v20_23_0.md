# Continuation prompt — v20.23.0

We are continuing the Football Board Sandbox Game Engine migration from build `v20.23.0`.
...
- Attack End Turn enters defense and does not reset turn-scoped state. Defense End Turn starts the next numbered turn when it exists; final defense enters `complete`.
...
- Before a normal MOVE has physically moved, all gameplay controls except `CANCEL MOVE` are blocked. The Engine independently rejects all other commands.
- Inactive Tracker phase disables only normal gameplay card actions. Selection and inspection remain possible for both teams. Free tools, INACTIVE and card flips remain available. A canonical Bonus Action owner remains the exception.
...
- `BONUS_ACTION_ENDED` is the canonical closure command. It accepts ready (explicit decline), active (including a partial Bonus MOVE), and awaiting-end continuation states. It validates optional continuation identity, removes the canonical continuation, and emits the established `BONUS_ACTION_DECLINED` or `BONUS_ACTION_ENDED` Timeline event with existing AI metadata.
- `advance-turn` starts the designated team’s next numbered turn and resets its turn-scoped state. A requested turn beyond the configured maximum enters `complete`; do not clamp back to the final turn. `resume-phase` returns to its named phase without a Tracker reset.
- The Bonus Action transaction is atomic for Undo/Redo, including End B.A.
...
Verification status for v20.23.0:

- Focused Engine/Controller/Match/Timeline/Tracker tests passed: `node --test src/engine/*.test.mjs src/match/*.test.mjs src/timeline/*.test.mjs src/tracker/*.test.mjs` (108 passing).
