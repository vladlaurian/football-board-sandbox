# Continuation prompt — v20.20.1

We are continuing the Football Board Sandbox Game Engine migration from build `v20.20.1`.
...
- v20.20.1: Group Move UI/turn-closure correction
...
Group Move final contract in v20.20.1:

- Pressing GROUP MOVE opens a UI-only, full-width zone preview that is positioned by dragging the band; it consumes nothing.
...
- The frozen zone remains in canonical MatchState for Engine validation but disappears visually after confirmation. In-zone active-team candidates are visibly marked: eligible players are highlighted; ineligible players have a grey outline and lock.
- Cursor preview calls the pure Group Move Engine evaluator, so it uses Group maximum distance and direction, not card Speed or normal-MOVE path blocking.
- End Turn clears the active Group Move state before Timeline records the next phase. The Group Move lock must never block the opposing team.
...
Verification status for v20.20.1:

- Focused tests passed: `node --test src/engine/*.test.mjs src/rules/ruleSets.test.mjs src/tracker/*.test.mjs src/timeline/aiAnalysisExport.test.mjs src/multiplayer/freeToolsAuthority.test.mjs` (60 passing).
