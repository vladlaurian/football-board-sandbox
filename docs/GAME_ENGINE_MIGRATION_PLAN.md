...
The Rule Set schema was version 4 for the original migration and added Group Move settings. As of v20.52.4, schema v6 owns `maxPlayers`, `zoneLength`, `maxOrthogonalDistance`, `maxDiagonalDistance`, and `sameDirectionOnly`; old Rule Sets receive an explicit one-time normalization migration from former `maxDistance` into both geometry limits. MatchContext freezes these settings at Match start. The Engine owns zone eligibility, one move per player, maximum player count, geometry-specific maximum distance, first-move orientation/direction, player and ball destination restrictions, and the deliberate Group Move exception that may cross players. End Turn retains the existing turn reset and is the only normal closure. Manual Multiplayer and Editor Mode remain unchanged.
...
v20.52.4 applies the first new mechanic rule through the Phase 10 projection contract: Group Move's Rule Set has separate orthogonal and diagonal limits (`6` / `4` by default), frozen in MatchContext and copied into canonical confirmed Group Move state. The pure Engine evaluator selects and returns the applicable limit; the UI displays that projection rather than deriving it. Legacy Rule Sets and legacy saved Group Move state migrate a former single limit into both values. Timeline semantics and AI vocabulary remain `GROUP_MOVE_ACTIVATED` / `GROUP_MOVE_PIECE`; activation metadata now records both frozen limits. Manual Multiplayer is unchanged.
...
### v20.56.21 — Formation-template and Adjust presentation correction

**Status:** Complete.

This corrective Workspace build starts from the accepted v20.56.19 source. The
fixed Blue templates and Red mirrors are tested to remain in their respective
halves and outside the rendered centre-circle geometry; attackers remain as
advanced as that constraint permits. Adjust is no longer a role-zone placement
algorithm: it presents only a selected starter's local formation-anchor 5×5
area, with no automatic piece movement. Existing coordinate-display preference
is not touched. Formation-role warnings use correct card metadata and an
