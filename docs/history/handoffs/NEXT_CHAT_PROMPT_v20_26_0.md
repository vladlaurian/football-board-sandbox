# Continuation prompt — v20.26.0

We are continuing the Football Board Sandbox Game Engine migration from build `v20.26.0`.
...
- v20.26.0: Engine-owned Single Player Pass route confirmation and canonical plan
...
- Route confirmation validates the selected route against MatchContext path mode, rejects a blocked origin before action consumption, builds the canonical Pass plan from MatchState + MatchContext, then consumes one normal Tracker action or no Bonus Tracker action.
- The plan contains selected origin, foot, distance, Long/Short classification, requested/effective target, direct hit, defensive crossings and ordered interceptors.
- Confirming the route may only place the canonical state into `completing`, `awaiting-interceptor-choice`, or `awaiting-interception-roll`. It does not choose an interceptor, submit/consume dice, resolve a result, move ball, change possession or create/close a Bonus Action.
- Normal Pass start/target/route entries are stepwise History. Bonus Pass start/target/route entries remain one atomic continuation transaction.
- The remaining legacy downstream choice/roll/resolution code reads the Engine-built canonical plan temporarily. Manual Multiplayer remains unchanged.

Deferred approved goalkeeper rule:
...
- This is deliberately not implemented in v20.26.0. It belongs in a separate amendment of `PASS_TARGET_SELECTED`.
- A goalkeeper remains a physical player: they may block a route and may be the first player hit by a route aimed elsewhere.
...
- Audit and propose only Engine-owned Pass interceptor choice (`PASS_INTERCEPTOR_SELECTED`): validation of the explicit canonical pending choice, deterministic reorder/modifier update, and creation of the next pending-roll descriptor. Do not migrate die submission, delayed resolution, interception outcome, possession or Pass completion unless separately approved after audit.
- Do not silently bundle the deferred goalkeeper target rule. It needs its own explicit approved amendment of `PASS_TARGET_SELECTED`.

Verification status for v20.26.0:

- Focused Engine/Controller/Manual-Multiplayer-guard tests passed: `node --test src/engine/gameEngine.test.mjs src/engine/singlePlayerController.test.mjs src/multiplayer/actionStartAuthority.test.mjs` (63 passing).
- Full `npm test` and `npm run build` must be rerun in an environment with dependencies installed. This archive intentionally does not contain `node_modules`; current local environment lacks `react` and `vite`.
