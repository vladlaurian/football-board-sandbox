# Continuation prompt — v20.37.0

Continue the Football Board Sandbox Game Engine migration from `v20.37.0`.
...
- `CardVisualCanvas.jsx` owns the shared visual card renderer used by Editor, Inspector, Assign Card and PNG export. `main.jsx` supplies render helpers and existing layout callbacks through `CardPreview` context.
...
## v20.37.0 verification

- Focused extracted-component test: `node --test src/board/extractedComponents.test.mjs`.
- Full verification remains required before delivery: `npm test` and `npm run build`.

## Exact manual tests for v20.37.0

1. Open Cards in Editor Mode. Select a card and verify both Front and Back previews render exactly as before.
2. In Layout Zones, select a zone, drag it, resize it and confirm the displayed live coordinates, selected outline and saved position behave exactly as before.
3. Add a custom front or back layout zone; move, resize and delete it. Confirm the visual result and existing controls are unchanged.
4. Inspect a player card outside the Cards panel and flip it, if flip is enabled. Confirm the card renders correctly.
5. Open Assign Card, select a card and flip its preview. Confirm the preview is correct and Assign still works.
6. Export Front PNG and Back PNG for a card. Confirm both use the same visual card rendering as the editor.
7. Confirm an ordinary Manual Multiplayer session retains its established card previews and card assignment behavior.
