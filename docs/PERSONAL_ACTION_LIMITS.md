...
Editor Mode is an unrestricted workspace, not Match gameplay. Its same panel is a manually clickable workspace marker with all three slots available for both teams; it never infers attacking or defending roles and never invokes Engine validation. Crossing from Match to Editor deliberately clears the just-closed Match's map, so Editor starts with no inherited puck dots; any subsequent Editor marker is manual. Manual Multiplayer does not receive this control or the Single Player enforcement rule.
